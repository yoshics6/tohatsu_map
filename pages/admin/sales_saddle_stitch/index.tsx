...
} from "@/components/table/admin/sales_saddle_stitch/TableHeads";
import {
  getSalesSaddleStitch,
  deleteSalesSaddleStitch,
  deleteAllSalesSaddleStitch,
  getSalesSaddleStitchById,
} from "@/features/admin/sales_saddle_stitch";
import saveAsExcel from "@/features/admin/sales_saddle_stitch/export";
// import exportPDFPublic from "@/features/admin/sales_saddle_stitch/export_pdf";
import { Field, Form, Formik, FormikProps, useFormik } from "formik";
import { TextField as TextFieldInput } from "formik-material-ui";
import Textarea from "@mui/joy/Textarea";
const SalesSaddleStitch: NextPage = () => {
  const dispatch = appDispatch();
  const [searched, setSearched] = React.useState<string>("");
  const { data } = appSelector((state) => state.sales_saddle_stitch);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getSalesSaddleStitch(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getSalesSaddleStitch(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_sadd_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesSaddleStitch(data)).then((result: any) => {
              dispatch(getSalesSaddleStitch(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesSaddleStitch(id)).then((result: any) => {
              dispatch(getSalesSaddleStitch(""));
            });
          }
        );
  ...
  //   dispatch(getSalesSaddleStitchById(id)).then((value: any) => {
  //     exportPDFPublic({
  //       rows: [
  //         {
  ...
  //     formik.setValues({
  //       sals_sadd_id: value.payload[0].sals_sadd_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_sadd_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                      ...
                      .map((row: any, index) => {
                        const isItemSelected = isSelected(row.sals_sadd_id);
                        ...
                        return (
                          <TableRow
                            hover
                            // onClick={(event) =>
                            //   handleClick(event, row.sals_sadd_id)
                            // }
                            role="checkbox"
                            aria-checked={isItemSelected}
                            tabIndex={-1}
                            key={row.sals_sadd_id}
                          ...
                          >
                            ...
                            <TableCell align="center">
                              {row.sals_sadd_fullname}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_company_name}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_tel}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_sadd_doc_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_printing_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_amount?.toLocaleString(undefined, {
                                maximumFractionDigits: 3,
                              })}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_quotation_request}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_finished_size}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_cover}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_sadd_cover_paper}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_text_paper}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_printing}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_cover_coating}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_text_coating}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_sadd_printing_volume?.toLocaleString(
                                undefined,
                                {
                                  maximumFractionDigits: 3,
                                }
                            ...
                            {
                            row.sals_sadd_quotation_request != 'No' ? 
                            <TableCell align="center">
                            ...
                            {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_sadd_id)}
                                >
                            ...
                            {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_sadd_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                  // onClick={() => exportspdf(formik.values.sals_sadd_id)}
                  // onBlur={() => setClose()}
                >
                  Download
                </Button>
...
export default withAuth(SalesSaddleStitch);
