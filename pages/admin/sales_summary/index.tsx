...
} from "@/components/table/admin/sales_summary/TableHeads";
import {
  getSalesSummary,
  deleteSalesSummary,
  deleteAllSalesSummary,
  getSalesSummaryById,
  UpdateSendSalesSummary,
} from "@/features/admin/sales_summary";
import saveAsExcel from "@/features/admin/sales_summary/export";
// import exportPDFPublic from "@/features/admin/sales_summary/export_pdf";
...
const SalesSummary: NextPage = () => {
  ...
  const { data } = appSelector((state) => state.sales_summary);
  ...
  React.useEffect(() => {
    dispatch(getSalesSummary(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesSummary(data)).then((result: any) => {
              dispatch(getSalesSummary(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesSummary(id)).then((result: any) => {
              dispatch(getSalesSummary(""));
            });
          }
        );
  ...
  const Send = (id: any) => {
    Swal.fire({
      title: "Are you sure to change the status of quotation sending?",
      text: "",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Successfully.", "", "success").then(
          function () {
            dispatch(UpdateSendSalesSummary(data)).then((result: any) => {
              dispatch(getSalesSummary(""));
            });
          }
        );
  ...
  //   dispatch(getSalesSummaryById(id)).then((value: any) => {
  //     exportPDFPublic({
  //       rows: [
  //         {
  ...
  //     formik.setValues({
  //       sals_id: value.payload[0].sals_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                    ...
                    .map((row: any, index) => {
                      const isItemSelected = isSelected(row.sals_id);
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="center">
                            {row.sals_company_name}
                          </TableCell>
                          <TableCell align="center">{row.sals_tel}</TableCell>
                          <TableCell align="center">
                            {row.sals_email}
                          </TableCell>
                          <TableCell align="center">
                          ...
                          <TableCell align="center">
                            {row.sals_printing_type}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_amount?.toLocaleString(undefined, { maximumFractionDigits: 3 })}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_quotation_request}
                          </TableCell>
                          {row.sals_quotation_request != 'No' ? 
                          <TableCell align="center">
                          ...
                          {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_id)}
                                >
                          ...
                          {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                // onClick={() => exportspdf(formik.values.sals_id)}
                // onBlur={() => setClose()}
                >
                  Download
                </Button>
