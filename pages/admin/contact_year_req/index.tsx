...
} from "@/components/table/admin/contact_year_req/TableHeads";
import {
  getContactYearReq,
  deleteContactYearReq,
  deleteAllContactYearReq,
  getContactYearReqById,
} from "@/features/admin/contact_year_req";
import saveAsExcel from "@/features/admin/contact_year_req/export";
import exportPDFPublic from "@/features/admin/contact_year_req/export_pdf";
...
const Contact: NextPage = () => {
  ...
  const { data } = appSelector((state) => state.contact_year_req);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getContactYearReq(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getContactYearReq(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteContactYearReq(data)).then((result: any) => {
              dispatch(getContactYearReq(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllContactYearReq(id)).then((result: any) => {
              dispatch(getContactYearReq(""));
            });
          }
        );
  ...
  const exportspdf = (id: any) => {
    dispatch(getContactYearReqById(id)).then((value: any) => {
      exportPDFPublic({
        rows: [
          {
      ...
      >
        {numSelected > 0 ? (
          ...
          <Typography
            ...
            component="div"
          >
            {`Contact > Model Year Info Request`}
          </Typography>
        )}
        {numSelected > 0 ? (
