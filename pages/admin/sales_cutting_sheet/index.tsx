...
} from "@/components/table/admin/sales_cutting_sheet/TableHeads";
import {
  getSalesCuttingSheet,
  deleteSalesCuttingSheet,
  deleteAllSalesCuttingSheet,
  getSalesCuttingSheetById,
} from "@/features/admin/sales_cutting_sheet";
import saveAsExcel from "@/features/admin/sales_cutting_sheet/export";
// import exportPDFPublic from "@/features/admin/sales_cutting_sheet/export_pdf";
import { Field, Form, Formik, FormikProps, useFormik } from "formik";
import { TextField as TextFieldInput } from "formik-material-ui";
import Textarea from "@mui/joy/Textarea";
const SalesCuttingSheet: NextPage = () => {
  const dispatch = appDispatch();
  const [searched, setSearched] = React.useState<string>("");
  const { data } = appSelector((state) => state.sales_cutting_sheet);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getSalesCuttingSheet(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getSalesCuttingSheet(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_cutt_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesCuttingSheet(data)).then((result: any) => {
              dispatch(getSalesCuttingSheet(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesCuttingSheet(id)).then((result: any) => {
              dispatch(getSalesCuttingSheet(""));
            });
          }
        );
  ...
  //   dispatch(getSalesCuttingSheetById(id)).then((value: any) => {
  //     exportPDFPublic({
  //       rows: [
  //         {
  ...
  //     formik.setValues({
  //       sals_cutt_id: value.payload[0].sals_cutt_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_cutt_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                      ...
                      .map((row: any, index) => {
                        const isItemSelected = isSelected(row.sals_cutt_id);
                        ...
                        return (
                          <TableRow
                            hover
                            // onClick={(event) =>
                            //   handleClick(event, row.sals_cutt_id)
                            // }
                            role="checkbox"
                            aria-checked={isItemSelected}
                            tabIndex={-1}
                            key={row.sals_cutt_id}
                          ...
                          >
                            ...
                            <TableCell align="center">
                              {row.sals_cutt_fullname}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_company_name}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_tel}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_cutt_doc_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_printing_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_amount?.toLocaleString(undefined, {
                                maximumFractionDigits: 3,
                              })}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_quotation_request}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_finished_size}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_page}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_text_paper}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_printing}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_text_coating}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_cutt_printing_volume?.toLocaleString(
                                undefined,
                                {
                                  maximumFractionDigits: 3,
                                }
                            ...
                            {
                            row.sals_cutt_quotation_request != 'No' ? 
                            <TableCell align="center">
                            ...
                            {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_cutt_id)}
                                >
                            ...
                            {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_cutt_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                  // onClick={() => exportspdf(formik.values.sals_cutt_id)}
                  // onBlur={() => setClose()}
                >
                  Download
                </Button>
...
export default withAuth(SalesCuttingSheet);
