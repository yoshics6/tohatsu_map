...
import saveAsExcel from "@/features/admin/find_dealer/export";

const Find_Dealer: NextPage = () => {
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("fd_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteFindDealer(data)).then((result: any) => {
              dispatch(getFindDealer(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllFindDealer(id)).then((result: any) => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.fd_id);
      ...
      <Box sx={{ width: "100%", mt: 3 }}>
        <Paper sx={{ width: "100%", mb: 2 }}>
          ...
          {/* <Button
            ...
            variant="contained"
            color="primary"
            onClick={() => router.push("/admin/find_dealer/add")}
          >
          ...
          <Button
            ...
            variant="contained"
            color="secondary"
            onClick={() => router.push("/admin/find_dealer/upload")}
          >
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                    ...
                    .map((row: any, index) => {
                      const isItemSelected = isSelected(row.fd_id);
                          ...
                          {/* <TableCell align="center">
                            ...
                            >
                              <IconButton
                                ...
                                onClick={() =>
                                  router.push(
                                    `/admin/find_dealer/edit?id=${row.fd_id}`
                                  )
                                }
                              >
...
export default withAuth(Find_Dealer);
