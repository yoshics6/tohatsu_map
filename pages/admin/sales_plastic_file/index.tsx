...
} from "@/components/table/admin/sales_plastic_file/TableHeads";
import {
  getSalesPlasticFile,
  deleteSalesPlasticFile,
  deleteAllSalesPlasticFile,
  getSalesPlasticFileById,
} from "@/features/admin/sales_plastic_file";
import saveAsExcel from "@/features/admin/sales_plastic_file/export";
// import exportPDFPublic from "@/features/admin/sales_plastic_file/export_pdf";
import { Field, Form, Formik, FormikProps, useFormik } from "formik";
import { TextField as TextFieldInput } from "formik-material-ui";
import Textarea from "@mui/joy/Textarea";
const SalesPlasticFile: NextPage = () => {
  const dispatch = appDispatch();
  const [searched, setSearched] = React.useState<string>("");
  const { data } = appSelector((state) => state.sales_plastic_file);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getSalesPlasticFile(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getSalesPlasticFile(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_plas_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesPlasticFile(data)).then((result: any) => {
              dispatch(getSalesPlasticFile(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesPlasticFile(id)).then((result: any) => {
              dispatch(getSalesPlasticFile(""));
            });
          }
        );
  ...
  //     formik.setValues({
  //       sals_plas_id: value.payload[0].sals_plas_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_plas_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                      ...
                      .map((row: any, index) => {
                        const isItemSelected = isSelected(row.sals_plas_id);
                        ...
                        return (
                          <TableRow
                            hover
                            // onClick={(event) =>
                            //   handleClick(event, row.sals_plas_id)
                            // }
                            role="checkbox"
                            aria-checked={isItemSelected}
                            tabIndex={-1}
                            key={row.sals_plas_id}
                          ...
                          >
                            ...
                            <TableCell align="center">
                              {row.sals_plas_fullname}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_company_name}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_tel}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_plas_doc_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_printing_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_amount?.toLocaleString(undefined, {
                                maximumFractionDigits: 3,
                              })}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_quotation_request}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_finished_size}
                            </TableCell>
                            {/* <TableCell align="center">
                              {row.sals_plas_page}
                            </TableCell> */}
                            ...
                            <TableCell align="center">
                              {row.sals_plas_printing}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_binding}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_plas_printing_volume?.toLocaleString(
                                undefined,
                                {
                                  maximumFractionDigits: 3,
                                }
                            ...
                            {
                            row.sals_plas_quotation_request != 'No' ? 
                            <TableCell align="center">
                            ...
                            {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_plas_id)}
                                >
                            ...
                            {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_plas_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                  // onClick={() => exportspdf(formik.values.sals_plas_id)}
                  // onBlur={() => setClose()}
                >
                  Download
                </Button>
...
export default withAuth(SalesPlasticFile);
