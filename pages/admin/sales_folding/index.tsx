...
} from "@/components/table/admin/sales_folding/TableHeads";
import {
  getSalesFolding,
  deleteSalesFolding,
  deleteAllSalesFolding,
  getSalesFoldingById,
} from "@/features/admin/sales_folding";
import saveAsExcel from "@/features/admin/sales_folding/export";
// import exportPDFPublic from "@/features/admin/sales_folding/export_pdf";
import { Field, Form, Formik, FormikProps, useFormik } from "formik";
import { TextField as TextFieldInput } from "formik-material-ui";
import Textarea from "@mui/joy/Textarea";
const SalesFolding: NextPage = () => {
  const dispatch = appDispatch();
  const [searched, setSearched] = React.useState<string>("");
  const { data } = appSelector((state) => state.sales_folding);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getSalesFolding(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getSalesFolding(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_fold_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesFolding(data)).then((result: any) => {
              dispatch(getSalesFolding(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesFolding(id)).then((result: any) => {
              dispatch(getSalesFolding(""));
            });
          }
        );
  ...
  //   dispatch(getSalesFoldingById(id)).then((value: any) => {
  //     exportPDFPublic({
  //       rows: [
  //         {
  ...
  //     formik.setValues({
  //       sals_fold_id: value.payload[0].sals_fold_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_fold_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                      ...
                      .map((row: any, index) => {
                        const isItemSelected = isSelected(row.sals_fold_id);
                        ...
                        return (
                          <TableRow
                            hover
                            // onClick={(event) =>
                            //   handleClick(event, row.sals_fold_id)
                            // }
                            role="checkbox"
                            aria-checked={isItemSelected}
                            tabIndex={-1}
                            key={row.sals_fold_id}
                          ...
                          >
                            ...
                            <TableCell align="center">
                              {row.sals_fold_fullname}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_company_name}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_tel}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_fold_doc_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_printing_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_amount?.toLocaleString(undefined, {
                                maximumFractionDigits: 3,
                              })}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_quotation_request}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_finished_size}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_open_size}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_column}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_page}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_text_paper}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_printing}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_text_coating}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_fold_printing_volume?.toLocaleString(
                                undefined,
                                {
                                  maximumFractionDigits: 3,
                                }
                            ...
                            {
                            row.sals_fold_quotation_request != 'No' ? 
                            <TableCell align="center">
                            ...
                            {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_fold_id)}
                                >
                            ...
                            {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_fold_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                  // onClick={() => exportspdf(formik.values.sals_fold_id)}
                  // onBlur={() => setClose()}
                >
                  Download
                </Button>
...
export default withAuth(SalesFolding);
