...
} from "@/components/table/admin/sales_calendar/TableHeads";
import {
  getSalesCalendar,
  deleteSalesCalendar,
  deleteAllSalesCalendar,
  getSalesCalendarById,
} from "@/features/admin/sales_calendar";
import saveAsExcel from "@/features/admin/sales_calendar/export";
// import exportPDFPublic from "@/features/admin/sales_calendar/export_pdf";
import { Field, Form, Formik, FormikProps, useFormik } from "formik";
import { TextField as TextFieldInput } from "formik-material-ui";
import Textarea from "@mui/joy/Textarea";
const SalesCalendar: NextPage = () => {
  const dispatch = appDispatch();
  const [searched, setSearched] = React.useState<string>("");
  const { data } = appSelector((state) => state.sales_calendar);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getSalesCalendar(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getSalesCalendar(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_cale_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesCalendar(data)).then((result: any) => {
              dispatch(getSalesCalendar(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesCalendar(id)).then((result: any) => {
              dispatch(getSalesCalendar(""));
            });
          }
        );
  ...
  //     formik.setValues({
  //       sals_cale_id: value.payload[0].sals_cale_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_cale_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                    ...
                    .map((row: any, index) => {
                      const isItemSelected = isSelected(row.sals_cale_id);
                      ...
                      return (
                        <TableRow
                          hover
                          // onClick={(event) =>
                          //   handleClick(event, row.sals_cale_id)
                          // }
                          role="checkbox"
                          aria-checked={isItemSelected}
                          tabIndex={-1}
                          key={row.sals_cale_id}
                        ...
                        >
                          ...
                          <TableCell align="center">
                            {row.sals_cale_fullname}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_company_name}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_tel}
                          </TableCell>
                          ...
                          <TableCell align="center">
                            {row.sals_cale_doc_type}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_printing_type}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_amount?.toLocaleString(undefined, {
                              maximumFractionDigits: 3,
                            })}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_quotation_request}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_finished_size}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_page}
                          </TableCell>
                          ...
                          <TableCell align="center">
                            {row.sals_cale_printing}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_stand}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_binding}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_o_ring_color}
                          </TableCell>
                          <TableCell align="center">
                            {row.sals_cale_printing_volume?.toLocaleString(
                              undefined,
                              {
                                maximumFractionDigits: 3,
                              }
                          ...
                          {
                            row.sals_cale_quotation_request != 'No' ? 
                            <TableCell align="center">
                          ...
                          {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_cale_id)}
                                >
                          ...
                          {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_cale_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                // onClick={() => exportspdf(formik.values.sals_cale_id)}
                // onBlur={() => setClose()}
                >
                  Download
                </Button>
...
export default withAuth(SalesCalendar);
