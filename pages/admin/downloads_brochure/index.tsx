...
const DownloadsBrochure: NextPage = () => {
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getDownloadsBrochure(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getDownloadsBrochure(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("db_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteDownloadsBrochure(data)).then((result: any) => {
              dispatch(getDownloadsBrochure(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllDownloadsBrochure(id)).then((result: any) => {
              dispatch(getDownloadsBrochure(""));
            });
          }
        );
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const DownloadsBrochureelected = rows.map((n: any) => n.db_id);
      setSelected(DownloadsBrochureelected);
      return;
    }
    setSelected([]);
  };
      ...
      <Box sx={{ width: "100%", mt: 3 }}>
        <Paper sx={{ width: "100%", mb: 2 }}>
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            onClick={() => router.push("/admin/downloads_brochure/add")}
          >
            Add Brochure
          </Button>
          {/* <Button
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* <Image
                              ...
                              alt={row.DownloadsBrochure_title}
                              width={150}
                              height={100}
                            /> */}
...
//   (store) => async () => {
//     const data: any = await store.dispatch(getDownloadsBrochureEN());
//     console.log(data);
//     return {
//       props: {
...
export default withAuth(DownloadsBrochure);
