...
} from "@/components/table/admin/sales_envelope/TableHeads";
import {
  getSalesEnvelope,
  deleteSalesEnvelope,
  deleteAllSalesEnvelope,
  getSalesEnvelopeById,
} from "@/features/admin/sales_envelope";
import saveAsExcel from "@/features/admin/sales_envelope/export";
// import exportPDFPublic from "@/features/admin/sales_envelope/export_pdf";
import { Field, Form, Formik, FormikProps, useFormik } from "formik";
import { TextField as TextFieldInput } from "formik-material-ui";
import Textarea from "@mui/joy/Textarea";
const SalesEnvelope: NextPage = () => {
  const dispatch = appDispatch();
  const [searched, setSearched] = React.useState<string>("");
  const { data } = appSelector((state) => state.sales_envelope);
  ...
  // *************************** Use Effect ***************************
  React.useEffect(() => {
    dispatch(getSalesEnvelope(""));
  }, [dispatch]);

  React.useEffect(() => {
    dispatch(getSalesEnvelope(searched));
  }, [dispatch, searched]);
  // *************************** Use Effect ***************************
  ...
  const Delete = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        data.append("sals_enve_id", id);
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteSalesEnvelope(data)).then((result: any) => {
              dispatch(getSalesEnvelope(""));
            });
          }
        );
  ...
  const DeleteAll = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted!", "Your data has been deleted.", "success").then(
          function () {
            dispatch(deleteAllSalesEnvelope(id)).then((result: any) => {
              dispatch(getSalesEnvelope(""));
            });
          }
        );
  ...
  //     formik.setValues({
  //       sals_enve_id: value.payload[0].sals_enve_id,
  //       contact_date: value.payload[0].contact_date,
  //       contact_fullname: value.payload[0].contact_fullname,
  //       contact_company_name: value.payload[0].contact_company_name,
  //       contact_phone_number: value.payload[0].contact_phone_number,
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n: any) => n.sals_enve_id);
              ...
              <TableBody>
                {rows.length > 0
                  ? stableSort(rows, getComparator(order, orderBy))
                      ...
                      .map((row: any, index) => {
                        const isItemSelected = isSelected(row.sals_enve_id);
                        ...
                        return (
                          <TableRow
                            hover
                            // onClick={(event) =>
                            //   handleClick(event, row.sals_enve_id)
                            // }
                            role="checkbox"
                            aria-checked={isItemSelected}
                            tabIndex={-1}
                            key={row.sals_enve_id}
                          ...
                          >
                            ...
                            <TableCell align="center">
                              {row.sals_enve_fullname}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_company_name}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_tel}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_enve_doc_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_printing_type}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_amount?.toLocaleString(undefined, {
                                maximumFractionDigits: 3,
                              })}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_quotation_request}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_finished_size}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_page}
                            </TableCell>
                            ...
                            <TableCell align="center">
                              {row.sals_enve_printing}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_coating}
                            </TableCell>
                            <TableCell align="center">
                              {row.sals_enve_printing_volume?.toLocaleString(
                                undefined,
                                {
                                  maximumFractionDigits: 3,
                                }
                            ...
                            {
                            row.sals_enve_quotation_request != 'No' ? 
                            <TableCell align="center">
                            ...
                            {/* <TableCell align="center">
                              ...
                              >
                                <IconButton
                                  ...
                                  aria-label="edit"
                                  size="large"
                                  onClick={() => openViewModal(row.sals_enve_id)}
                                >
                            ...
                            {/* <TableCell align="center">
                              <IconButton
                                ...
                                aria-label="edit"
                                size="large"
                                onClick={() => exportspdf(row.sals_enve_id)}
                                onBlur={() => setClose()}
                              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} mt={4}>
                <Button
                  ...
                  color="info"
                  fullWidth
                  // onClick={() => exportspdf(formik.values.sals_enve_id)}
                  // onBlur={() => setClose()}
                >
                  Download
                </Button>
...
export default withAuth(SalesEnvelope);
