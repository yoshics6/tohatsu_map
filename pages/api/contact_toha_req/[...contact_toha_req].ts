...
router.get(
  "/api/contact_toha_req/lists",
  async (req: NextApiRequest, res: NextApiResponse, next: any) => {
    try {
      const [response]: any = await connection.query(
        `SELECT * FROM contact_toha_req ORDER BY contact_date DESC`
      );
...
router.get(
  "/api/contact_toha_req/getbyid",
  async (req: NextApiRequest, res: NextApiResponse, next: any) => {
    const { id } = req.query;
    try {
      const [response]: any = await connection.query(
        `SELECT * FROM contact_toha_req WHERE contact_id = ?`,
        [id]
      );
      res.status(200).json({ status: "success", data: response });
    } catch {
...
router.post(
  "/api/contact_toha_req/delete",
  async (req: NextApiRequest, res: NextApiResponse) => {
    const form = formidable();
    form.parse(req, async (err, fields, files) => {
      const { contact_id } = fields;
      await connection.query(
        `DELETE FROM contact_toha_req WHERE contact_id = ${contact_id}`
      );
...
router.post(
  "/api/contact_toha_req/deleteall",
  async (req: NextApiRequest, res: NextApiResponse) => {
    const form = formidable();
    form.parse(req, async (err, fields, files) => {
      const { contact_id } = fields;
      let id = contact_id.toString();
      await connection.query(`DELETE FROM contact_toha_req WHERE contact_id IN (${id})`);
      res.status(200).json({ status: "success" });
    });
  }
);
