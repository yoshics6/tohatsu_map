...
import { TextField, FormControl, Button, InputLabel, Select, MenuItem, Grid } from "@mui/material";
...
const Maps = (props: any) => {
  ...
  var search_bt = DataProps.search.fd_busines_type != undefined ? DataProps.search.fd_busines_type : '';
  var search_lat = DataProps.search.location_lat != undefined ? DataProps.search.location_lat : 32.90691;
  var search_long = DataProps.search.location_long != undefined ? DataProps.search.location_long : -96.413837;
  ...
  const [initialPosition, setInitialPosition] = React.useState<[number, number]>([32.90691, -96.413837]);
  const [selectedPosition, setSelectedPosition] = React.useState<[number, number]>([32.90691, -96.413837]);
      ...
      >
        ...
        <MarkerClusterGroup>
          {rows.length > 0 ? rows.map((value: any) => {
            if (value.fd_busines_type.toLowerCase().search(search_bt.toLowerCase()) != -1) {
              ...
              return (
                ...
                >
                  <Popup>
                    ...
                    <p>{`Address : ${value.fd_address + ' ,' + value.fd_road + ' ,' + value.fd_subdistrict + ' ,' + value.fd_district + ' ,' + value.fd_province + ' ,' + value.fd_zipcode}`}</p>
                    <p>{`Tel : ${value.fd_tel}`}</p>
                    <p>{`Distance : ${distances}`}</p>
                    <a style={{ fontSize: "25px" }} target="_blank" href={'https://google.com/maps/search/' + lat + ',' + long}>View on map</a>
                  </Popup>
                </Marker>
              );
            }
